...
export async function fetchSpotifyOembed(input: string): Promise<SpotifyMeta> {
  ...
  if (!url) {
    throw new Error('URL do Spotify invalida.')
  }

  const oembedUrl = `https://open.spotify.com/oembed?url=${encodeURIComponent(url)}`
  const res = await fetch(oembedUrl)
  if (!res.ok) {
    throw new Error(`Falha no oEmbed do Spotify: ${res.status}`)
  }
...
async function getAccessToken(clientId: string, clientSecret: string): Promise<string> {
  ...
  if (!safeClientId || !safeClientSecret) {
    throw new Error('Credenciais do Spotify ausentes ou invalidas.')
  }
  ...
  if (!res.ok) {
    throw new Error(`Falha ao obter token do Spotify: ${res.status}`)
  }
...
export async function searchSpotifyTracks(
  ...
  if (!res.ok) {
    ...
    throw new Error(`Falha no search do Spotify: ${res.status}`)
  }
